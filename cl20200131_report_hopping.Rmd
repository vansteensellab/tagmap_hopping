...
Another project that was recently started by Matthias involves random transposon
...
In this first experiment Matthias added different concentrations of the
...
read_pairs <- function(row){
    ...
    fwd_dt = fread(paste0('/DATA/usr/c.leemans/projects/tagmap_hopping/',
                          'results/cl20200122_hopping_mathias/', row$genome,
                          '/hopping/overlap/', row$Forward, '.txt'), sep='\t',
                   col.names=col_vec, colClasses=c('V11'='character'))
    rev_dt = fread(paste0('/DATA/usr/c.leemans/projects/tagmap_hopping/',
                          'results/cl20200122_hopping_mathias/', row$genome,
                          '/hopping/overlap/', row$Reverse, '.txt'), sep='\t',
                   col.names=col_vec, colClasses=c('V11'='character'))
    fwd_dt[,side:='Forward']
    rev_dt[,side:='Reverse']
...
read_insertions <- function(row){
    fread(paste0('/DATA/usr/c.leemans/projects/tagmap_hopping/',
                 'results/cl20200122_hopping_mathias/', row$genome,
                 '/home/insertions/', row$home_location, '.txt'),
                 sep='\t')
}
...
pairing_dt[, genome:=ifelse(home_sequence=='pJOYC037', 'mm10', 'hg38')]
...
read_pair_dt = pairing_dt[,read_pairs(.SD), by='ID']
read_pair_dt[,p_adj:=as.numeric(p_adj)]

## this is a file with putative integrations of the PB home and a score
## for how likely these integrations are real.
insertions_hg38 = pairing_dt[genome=='hg38',read_insertions(.SD), by='ID']
insertions_mm10 = pairing_dt[genome=='mm10',read_insertions(.SD), by='ID']

col_vec = colnames(insertions_hg38)[colnames(insertions_hg38) %in%
                                    colnames(insertions_mm10) &
                                    !colnames(insertions_hg38) %in%
                                    c('start', 'end')]

insertions = rbind(insertions_hg38[,..col_vec], insertions_mm10[,..col_vec])

insertion_gr = makeGRangesFromDataFrame(insertions,
...
```{r}
overlap_count = read_pair_dt[,data.table(table(overlap)),by=c('ID', 'side')]
...
```{r}
distance_dt = read_pair_dt[overlap=='.', get_distance(.SD, insertion_gr),by='ID']
